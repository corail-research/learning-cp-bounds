    ...
    if (write_samples) {
        for (int index_size = start_size; index_size < number_of_sizes; index_size++) {
            ...
            while (std::getline(inputFilea, line)) {
                ...
                std::ofstream outputFilea("/Users/dariusdabert/Documents/Documents/X/3A/Stage Polytechnique Montréal/learning-bounds/data/ssp/train/ssp-data-trainset"+ sizes[index_size]+"-subnodes" + std::to_string(j) +  ".txt");
